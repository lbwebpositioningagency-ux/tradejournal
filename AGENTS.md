...
- **Mappe a intensità** (calendario del mese e griglia mensile, entrambi nella Dashboard): tinte dalla scala condivisa `--heat-*` via `heatTone()` in `src/lib/heat-scale.ts`, mai `bg-profit/NN`. Il testo sopra le tinte è `text-heat-foreground` / `text-heat-muted` (il `muted-foreground` del tema sulla tinta più forte sta sotto AA). L'intensità la decide solo `returnIntensity`. La Stagionalità NON usa questa scala: ha la sua taratura in `src/components/seasonality/accento.ts`.
