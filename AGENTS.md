...
- **Numeri a schermo: un solo formattatore**, `formatNumber` / `formatQuantity` in `src/lib/format-number.ts` (it-IT, punto delle migliaia anche a quattro cifre). `money.ts` e `instruments.ts` ci stanno sopra. Mai `toFixed`, `toLocaleString` o `replace(".", ",")` per mostrare un numero: `toFixed` resta lecito solo per serializzare un Decimal in stringa. Nelle tabelle `ui/table` le cifre sono tabulari e una cella numerica si allinea a destra da sola (con la sua intestazione).
- **404 veri**: un `notFound()` risponde 404 solo se scatta prima dello streaming. Nessun `loading.tsx` sopra una rotta dinamica: il controllo d'esistenza sta in un `layout.tsx` del segmento (vedi `trades/[id]/layout.tsx`), le liste stanno in un gruppo `(lista)`/`(mese)`/`(indice)`. Lo sorveglia `src/app/not-found-streaming.test.ts`.
- **Sistema visivo unico (v2, 14/09/2026)** — le decisioni stanno nella tavola «Sistema visivo v2 - applicato» del progetto Claude Design del referto 360; si cambiano lì, poi nel codice.
  - **Un carattere**: Geist (Geist Mono solo per codici e orari), caricato una volta in `app/layout.tsx`. Nessun `next/font` nelle pagine. Scala: 11 (solo etichette maiuscole) · 12 · 14 · 16 · 20 (titolo di pagina) · 24→30 (un KPI); niente `text-[Npx]`.
  - **Una palette**: i token oklch di `globals.css`. I `--md-*` del desk sono ALIAS dei token dell'app (`listino.css`), mai esadecimali. Il colore significa su/giù (`--profit`/`--loss`), attenzione (`--warning`) o azione (`--primary`); asset e finestre usano `--data-*`. Testo colorato solo su background e card.
  - **Un componente per compito**: testata `components/layout/page-header.tsx`, schede `components/layout/tab-nav.tsx` (sezioni del desk: `MacroDeskTabs`), segmentato `ui/segmented.tsx` (link) e `ui/segmented-control.tsx` (pulsanti).
  - **Tabelle larghe** del listino (13+ colonne): il CSS stringe il padding e ferma la prima colonna; il minimo di 11px non si tocca.
  - **Eccezione dichiarata**: la Stagionalità è congelata e resta fuori (Inter/JetBrains, `.macro-report` scuro fisso, vecchia barra delle sezioni) finché non la si sblocca.
...
- **Mappe a intensità** (calendario di `/day`, griglia mensile e mini-calendario della Dashboard): tinte dalla scala condivisa `--heat-*` via `heatTone()` in `src/lib/heat-scale.ts`, mai `bg-profit/NN`. Il testo sopra le tinte è `text-heat-foreground` / `text-heat-muted` (il `muted-foreground` del tema sulla tinta più forte sta sotto AA). L'intensità la decide solo `returnIntensity`. La Stagionalità NON usa questa scala: ha la sua taratura in `src/components/seasonality/accento.ts`.
