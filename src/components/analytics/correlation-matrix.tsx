 ...
 *
 * Ogni cella scrive DUE cose visibili, non in un tooltip: il coefficiente e le
 * settimane in cui le due strategie hanno operato insieme — le stesse su cui
 * il coefficiente è calcolato. Sotto la soglia il
 * coefficiente non c'è: bordo tratteggiato, settimane in comune e quante ne
 * servono.
 *
...
function cellTitle(pair: CorrelationPair, rowLabel: string, colLabel: string) {
  if (pair.lowSample) {
    return `${rowLabel} e ${colLabel}: solo ${pair.common} settimane in comune, ne servono ${MIN}`;
  }
  if (pair.r === null) {
    return `${rowLabel} e ${colLabel}: una delle due serie è piatta, la correlazione non è definita`;
  }
  const tone = correlationTone(pair);
  return `${rowLabel} e ${colLabel}: ${formatRatio(pair.r)}, ${tone ? CORRELATION_TONE_LABELS[tone] : ""} su ${pair.common} settimane in comune`;
}
    ...
    <div className="flex flex-col gap-3">
      <div className="overflow-x-auto">
        <table className="border-separate border-spacing-1 text-sm tabular-nums">
          <caption className="sr-only">
            Correlazione dei P&amp;L per settimana fra le strategie del periodo, con
            le settimane in cui ogni coppia ha operato insieme
          </caption>
          <thead>
            <tr>
      ...
      <ul className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-[var(--foreground-2)]" aria-label="Legenda">
        ...
        <LegendItem className="border border-dashed border-[var(--rule)]">
          meno di {MIN} settimane in comune
        </LegendItem>
      </ul>
    </div>
  );
 ...
 * soglia. Al posto di una griglia di celle vuote, la frase che lo dice e la
 * coppia più vicina con quante settimane ha e quante ne mancano.
 */
export function CorrelationUnavailable({
  ...
  /** La coppia con più settimane in comune. */
  closest: CorrelationPair;
  labels: Record<string, string>;
  /** Settimane complete coperte dallo storico nel periodo selezionato. */
  periods: number;
}) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-dashed border-[var(--rule)] p-4">
      <p className="text-sm font-semibold text-foreground">
        La correlazione non è calcolabile con lo storico attuale
      </p>
      <p className="max-w-[46rem] text-xs leading-5 text-[var(--foreground-2)]">
        Servono {MIN} settimane in cui due strategie operano entrambe. La coppia con
        più settimane in comune, {labels[closest.a]} × {labels[closest.b]}, ne ha{" "}
        {closest.common}: ne mancano {missing}. Lo storico del periodo copre {periods}{" "}
        {periods === 1 ? "settimana completa" : "settimane complete"}.
      </p>
      <div className="flex max-w-[28rem] items-center gap-3">
        ...
        <span className="shrink-0 text-xs tabular-nums text-muted-foreground">
          {closest.common} di {MIN} settimane
        </span>
      </div>
    </div>
  );
      ...
      <table className="w-full min-w-[36rem] text-sm tabular-nums">
        ...
        <thead>
          <tr className="border-b text-left text-xs text-muted-foreground">
            ...
            <th scope="col" className="py-1.5 pr-3 text-right font-medium">Settimane in comune</th>
            ...
            return (
              <tr key={`${p.a}|${p.b}`} className="border-b last:border-0">
                ...
                <td className="py-1.5 text-muted-foreground">
                  {p.lowSample
                    ? `campione insufficiente (servono ${MIN} settimane in comune)`
                    : tone
