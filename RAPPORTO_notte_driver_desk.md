...
1. **Commit estraneo sul branch.** Alle 23:27 una TUA sessione parallela (Claude Opus 5) ha committato `2fa7831` — "Budget del cron a 150s" (stagionalità: `src/lib/seasonality/job.ts` + PROGRESS.md) — mentre era attivo il checkout di `feature/driver-desk`: il commit è finito sul mio branch invece che su main. Non l'ho toccato (riscrivere la storia con un'altra sessione forse attiva mi è parso più rischioso). Se vuoi portarlo su main subito: `git checkout main && git cherry-pick 2fa7831`. La suite completa è verde anche con quel commit incluso.
